...
def menu():
    ...
    param_list = Initialize() # Initializing constant names, lists, etc. that are needed in the game.
    ...
    Draw_static_1(window, param_list) # Function "Draw_static_1" draws the stationary objects in the menu-screen of the game.
    while True:
        ...
        for event in pygame.event.get():
            if event.type == QUIT:
                ...
                environment_list.add(Environment(param_list[52], 10, 550))
                environment_list.add(Environment(param_list[52], 250, 500))
                environment_list.add(Environment(param_list[52], 990, 300))
                environment_list.add(Environment(param_list[54], 1090, 585))
                environment_list.add(Environment(param_list[53], 270, 230))
                environment_list.add(Environment(param_list[54], 600, 200))
                environment_list.add(Environment(param_list[52], 600, 400))
                ExitFlag = 1
                break
            ...
            # User clicks on the map on the top right corner to start palying the game with that map.
                ...
                environment_list.add(Environment(param_list[53], 535, 15))
                environment_list.add(Environment(param_list[52], 10, 580))
                environment_list.add(Environment(param_list[54], 40, 400))
                environment_list.add(Environment(param_list[52], 700, 140))
                environment_list.add(Environment(param_list[52], 650, 340))
                environment_list.add(Environment(param_list[54], 400, 50))
                environment_list.add(Environment(param_list[53], 190, 380))
                ExitFlag = 1
                break
            ...
            # User clicks on the map on the lower left corner to start playing the game with that map.
                ...
                environment_list.add(Environment(param_list[53], 1000, 20))
                environment_list.add(Environment(param_list[54], 850, 20))
                environment_list.add(Environment(param_list[52], 650, 50))
                environment_list.add(Environment(param_list[53], 400, 50))
                environment_list.add(Environment(param_list[52], 20, 570))
                environment_list.add(Environment(param_list[54], 1100, 570))
                environment_list.add(Environment(param_list[52], 50, 90))
                environment_list.add(Environment(param_list[54], 170, 370))
                ExitFlag = 1
                break
            ...
            # User clicks on the map on the lower right corner to start playing the game with that map.
                ...
                environment_list.add(Environment(param_list[52], 50, 530))
                environment_list.add(Environment(param_list[52], 1050, 530))
                environment_list.add(Environment(param_list[53], 210, 200))
                environment_list.add(Environment(param_list[54], 930, 210))
                environment_list.add(Environment(param_list[52], 800, 210))
                environment_list.add(Environment(param_list[54], 210, 70))
                environment_list.add(Environment(param_list[52], 570, 430))
                ExitFlag = 1
                break
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
                ...
                (tower, round_stats, delta, i) = Set_machine_gun(param_list, window, pieces_list, round_stats, delta, i, player, start, field, jumpLines, environment_list)
            ...
            for tower in param_list[15]: # Loop through all enemies.
            ...
            for tower in param_list[50]: # Loop through all the mines that are in the game field.
                if event.type == pygame.MOUSEBUTTONDOWN and xpos > (tower.rect.centerx-10) and xpos < (tower.rect.centerx+10) and ypos > (tower.rect.centery-10) and ypos < (tower.rect.centery+10):
                    (tower, round_stats, delta, i) = Draw_2(param_list, window, pieces_list, round_stats, delta, i, tower, player, start, field, jumpLines, environment_list)

            for tower in param_list[59]: # Loop through all the smart bombs that are in the game field.
                if event.type == pygame.MOUSEBUTTONDOWN and xpos > (tower.rect.centerx-10) and xpos < (tower.rect.centerx+10) and ypos > (tower.rect.centery-10) and ypos < (tower.rect.centery+10):
                    (tower, round_stats, delta, i) = Draw_2(param_list, window, pieces_list, round_stats, delta, i, tower, player, start, field, jumpLines, environment_list)
        ...
        if float(time.monotonic()) - delta > 0.5 and i < len(round_stats) and i != -1: # Add an enemy in the round_stats-list to a pygame.sprite.group()-type list.
            ...
            if len(round_stats) - i == 1: # If all enemies in the round_stats-list have already been added.
                ...
                i = i + 1
        Draw_static(window, param_list, pieces_list, player, environment_list) # Draw all the stationary ojects in the game screen.

        param_list[15].draw(window) # Draw all towers to the game field.
        param_list[50].draw(window) # Draw all mines to the game field.
        param_list[59].draw(window) # Draw all smart bombs to the game field.
        for enemy in param_list[17]: # Loop through all enemies in the game field.
            if enemy.direction == "up" or enemy.direction == "down":
                window.blit((param_list[51].render(str(enemy.health), True, BLACK)), (enemy.rect.centerx + 16, enemy.rect.centery - 7)) # Draw health of the enemy next to the enemy.
        ...
        for tower in param_list[50]: # Loop through all the mines in the field.
        ...
        for tower in param_list[59]: # Loop through all the smart bombs in the field.
        ...
        for bullet in param_list[64]: # Loop through all the missiles in the field.
        ...
        for tower in param_list[15]: # Loop through all the basic towers in the field.
            ...
            for enemy in param_list[17]: # Loop through all enemies in the field
                ...
                if enemy.dist <= tower.range: # If an enemy is within the range of a tower.
                    param_list[29].append(enemy) # Add the enemy to a list that includes all enemies that are within the range of a tower.
                    param_list[31][enemy.value] = enemy # Add a new key and a value corresponding to that key to a dictionary. The key is a "value"-variable of an enemy and the value is the enemy. The "enemy.value" is a variable that depends linearly on the length the enemy has travelled in the field.
                    param_list[30].append(enemy.value) # Add the enemy.value-variable to a distinct list.
            ...
            param_list[30].sort() # Sort the list that contains value-parameters of the enemies from smallest to largest.
            param_list[30].reverse()
            if float(time.monotonic()) - float(tower.beginning) >= tower.interval and (param_list[30]): # Check if enough time has passed from the previous shot so that we can shoot again.
                target = [param_list[31][param_list[30][0]].rect.centerx, param_list[31][param_list[30][0]].rect.centery] # Set the target to be the enemy that has travelled the longest distance.
                ...
                Type = tower.type
                Type_2 = tower.type_2
                bullet = Bullet(target, startpoint, damage, poison, Type, Type_2, param_list[31][param_list[30][0]]) # Create an object of class Bullet that represents a moving bullet.
