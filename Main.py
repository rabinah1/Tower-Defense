...
def main(pieces_list, player, field, environment_list, file, window, param_list, start, Menu_text):
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT:
            ...
            elif event.type == pygame.MOUSEBUTTONDOWN and xpos < 60 and ypos < 60: # Pause the game.
                ...
                while True:
                    n = 0
                    for event in pygame.event.get():
                        if event.type == pygame.MOUSEBUTTONDOWN and pygame.mouse.get_pos()[0] < 60 and pygame.mouse.get_pos()[1] < 60: # If the player clicks on the pause button, we will break out of this loop and continue playing the game.
                            n = 1
                            break
            ...
            for tower in param_list[51]: # Loop through all the mines that are in the game field.
                if event.type == pygame.MOUSEBUTTONDOWN and xpos > (tower.rect.centerx-10) and xpos < (tower.rect.centerx+10) and ypos > (tower.rect.centery-10) and ypos < (tower.rect.centery+10):
                    (tower, round_stats, delta, i) = Draw_2(param_list, window, pieces_list, round_stats, delta, i, tower, player, start, field, jumpLines, environment_list)

            for tower in param_list[60]: # Loop through all the smart bombs that are in the game field.
                if event.type == pygame.MOUSEBUTTONDOWN and xpos > (tower.rect.centerx-10) and xpos < (tower.rect.centerx+10) and ypos > (tower.rect.centery-10) and ypos < (tower.rect.centery+10):
                    (tower, round_stats, delta, i) = Draw_2(param_list, window, pieces_list, round_stats, delta, i, tower, player, start, field, jumpLines, environment_list)
        ...
        for tower in param_list[51]: # Loop through all the mines in the field.
        ...
        for tower in param_list[60]: # Loop through all the smart bombs in the field.
        ...
        for tower in param_list[15]: # Loop through all the basic towers in the field.
            ...
            for enemy in param_list[17]: # Loop through all enemies in the field
                ...
                if enemy.dist <= tower.range: # If an enemy is within the range of a tower.
                    param_list[30].append(enemy) # Add the enemy to a list that includes all enemies that are within the range of a tower.
                    param_list[32][enemy.value] = enemy # Add a new key and a value corresponding to that key to a dictionary. The key is a "value"-variable of an enemy and the value is the enemy. The "enemy.value" is a variable that depends linearly on the length the enemy has travelled in the field.
                    param_list[31].append(enemy.value) # Add the enemy.value-variable to a distinct list.
            ...
            param_list[31].sort() # Sort the list that contains value-parameters of the enemies from smallest to largest.
            param_list[31].reverse()
            if float(time.monotonic()) - float(tower.beginning) >= tower.interval and (param_list[31]): # Check if enough time has passed from the previous shot so that we can shoot again.
                ...
                Type = tower.type
                Type_2 = tower.type_2
                bullet = Bullet(target, startpoint, damage, poison, Type, Type_2, param_list[32][param_list[31][0]]) # Create an object of class Bullet that represents a moving bullet.
